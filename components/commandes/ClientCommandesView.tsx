  ...
  return (
    <div className="max-w-4xl mx-auto px-4 pt-8 pb-20 md:pb-12">
      ...
      <div className="flex items-end justify-between mb-8 flex-wrap gap-3">
        ...
        <Link href="/commandes/nouveau" className="hidden sm:inline-flex items-center gap-1.5 bg-orange-700 hover:bg-orange-800 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition">
          + Nouvelle commande
        </Link>
      </div>
      ...
      {commandes.length === 0 ? (
        <div className="flex flex-col items-center text-center py-24 gap-4">
          ...
          <Link href="/produits" className="bg-orange-700 hover:bg-orange-800 text-white font-semibold px-8 py-3 rounded-xl transition">Voir les produits</Link>
        </div>
      ) : (
        <div className="space-y-4">
          {commandes.map(commande => {
            ...
            return (
              <div key={commande.id} className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 overflow-hidden">
                <div
                ...
                >
                  <div className="flex items-center gap-4">
                    <div>
                      <p className="text-xs text-stone-400 dark:text-stone-500 mb-1">#{commande.id.slice(-8).toUpperCase()}</p>
                          ...
                          return (
                            <div key={s} className="flex items-center flex-1">
                              <div className="flex flex-col items-center flex-1">
                                ...
                                <p className={`text-xs mt-1 text-center hidden sm:block ${isDone ? 'text-orange-700 dark:text-orange-400 font-medium' : 'text-stone-400'}`}>{cfg.label}</p>
                              </div>
                              {i < 4 && <div className={`h-0.5 flex-1 ${i < currentIndex ? 'bg-orange-700' : 'bg-stone-200 dark:bg-stone-700'}`} />}
                            </div>
                          )
                        })}
                      </div>
                      ...
                      <div className="space-y-3">
                        {commande.items.map(item => (
                          <div key={item.id} className="flex items-start gap-3">
                            ...
                            <p className="font-semibold text-sm text-stone-800 dark:text-stone-100 shrink-0 tabular-nums">{(item.prix * item.quantite).toFixed(2)} DA</p>
                          </div>
                        ))}
                      </div>
                    </div>
