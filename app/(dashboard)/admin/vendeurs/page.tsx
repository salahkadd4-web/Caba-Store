...
import {
  ...
  cardSm, kpiCard, modalOverlay, modalBox, loadingPage, toastCls,
...
export default function AdminVendeursPage() {
  ...
  const doAction = async (id: string, action: string, extra: object = {}) => {
    ...
    const data = await res.json()
    if (res.ok) { showToast(data.message); await fetchVendeurs(); if (selected?.id === id) await fetchDetail(id); setShowDocModal(false); setAdminNote('') }
  ...
  const handleDocAction = async () => {
    ...
    const data = await res.json()
    if (res.ok) { showToast(data.message); await fetchDetail(selected.id); await fetchVendeurs() }
    else showToast(data.error || 'Erreur')
