      ...
      <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-2xl overflow-hidden">
        <div className="bg-stone-50 dark:bg-stone-800/60 px-5 py-3.5 flex gap-8">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="h-4 w-20 rounded bg-stone-200 dark:bg-stone-700" />
          ))}
        </div>
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="border-t border-stone-100 dark:border-stone-800 px-5 py-4 flex gap-6 items-center">
            <div className="w-10 h-10 rounded-xl bg-stone-200 dark:bg-stone-700 shrink-0" />
            ...
            <div className="h-6 w-20 rounded-full bg-stone-200 dark:bg-stone-700" />
