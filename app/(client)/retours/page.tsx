...
const RESOLUTIONS = [
  {
    ...
    label: "Remboursement",
    icon: Banknote,
    desc: "Recevoir le montant payé",
  },
  {
    value: "EXCHANGE",
    label: "Échange",
    icon: RefreshCw,
    desc: "Recevoir un produit de remplacement",
  },
  {
    value: "REPAIR",
    label: "Réparation",
    icon: Wrench,
    desc: "Faire réparer le produit",
  },
];
          ...
          {i < STEPS.length - 1 && (
            <div
              className={`flex-1 h-0.5 mx-2 mb-5 rounded-full transition-all duration-500 ${
                i < current ? "bg-orange-700" : "bg-stone-200 dark:bg-stone-700"
              }`}
            />
          )}
        </div>
...
function RetourContent() {
  ...
  useEffect(() => {
    fetch("/api/commandes")
      .then((r) => r.json())
      .then((data: Order[]) => {
        const livrees = data.filter(
          (c) => c.statut === "LIVREE" && !c.retourDemande,
        );
  ...
  const selectedItem =
    selectedOrder?.items.find((i) => i.id === selectedItemId) ?? null;
  ...
  const canNext =
    [!!selectedOrder, !!selectedItemId, !!reason && !!resolution][step] ?? true;
  ...
  if (result?.success) {
    const statusLabel =
      result.status === "APPROVED"
        ? { text: "Approuvée", color: "text-green-600 dark:text-green-400" }
        : result.status === "REJECTED"
          ? { text: "Refusée", color: "text-red-600 dark:text-red-400" }
          : {
      ...
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        <div className="bg-green-50 dark:bg-green-950 rounded-2xl p-8 border border-green-200 dark:border-green-800">
          ...
          <div className="bg-white dark:bg-stone-900 rounded-xl p-3 mb-4 border border-green-100 dark:border-green-900 space-y-1">
            ...
            <p className={`font-bold ${statusLabel.color}`}>
              {statusLabel.text}
            </p>
            {result.claimId && (
          ...
          <Link
            href="/commandes"
            className="inline-block px-6 py-2.5 bg-orange-700 text-white text-sm font-semibold rounded-xl hover:bg-orange-800 transition"
          >
            ← Mes commandes
          </Link>
        </div>
        ...
        <div className="bg-red-50 dark:bg-red-950 rounded-2xl p-8 border border-red-200 dark:border-red-800">
          ...
          <div className="flex gap-3 justify-center">
            <button
              ...
              className="px-5 py-2 bg-stone-900 dark:bg-white text-white dark:text-black rounded-xl text-sm font-semibold hover:opacity-80 transition"
            >
              Réessayer
            </button>
            <Link
              href="/commandes"
              className="px-5 py-2 border border-stone-200 dark:border-stone-700 text-stone-600 dark:text-stone-300 rounded-xl text-sm font-semibold hover:bg-stone-50 dark:hover:bg-stone-800 transition"
            >
              Mes commandes
            </Link>
          </div>
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        ...
        <p className="text-sm text-stone-500 mb-6">
          Les retours sont disponibles uniquement pour les commandes livrées.
        </p>
        <Link
          href="/commandes"
          className="text-orange-700 dark:text-orange-500 text-sm font-medium hover:underline"
        >
          ← Mes commandes
        </Link>
      </div>
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-10">
      <div className="mb-6">
        <Link
          href="/commandes"
          className="text-sm text-stone-400 hover:text-stone-600 dark:hover:text-stone-300 flex items-center gap-1 mb-4"
        >
          ← Retour
        </Link>
      ...
      {step === 0 && (
        <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-100 dark:border-stone-800 p-5">
          <p className="text-sm font-semibold text-stone-700 dark:text-stone-200 mb-4">
        ...
        <div className="space-y-3">
          ...
          <div className="bg-orange-50 dark:bg-orange-950/40 border border-orange-100 dark:border-orange-900 rounded-xl px-4 py-2.5 flex items-start gap-2">
            ...
            <p className="text-xs text-orange-700 dark:text-orange-500 leading-relaxed">
              Sélectionnez l&apos;article que vous souhaitez retourner. Une
              seule demande par article est possible.
            </p>
          </div>
                ...
                >
                  ...
                  <div
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${
                      isSelected
                        ? "border-orange-700 bg-orange-700 dark:border-orange-500 dark:bg-orange-600"
                  ...
                  <div className="relative w-12 h-12 bg-stone-100 dark:bg-stone-800 rounded-xl overflow-hidden shrink-0 flex items-center justify-center">
                    {image ? (
                      ...
                      <Package className="w-5 h-5 text-stone-300 dark:text-stone-600" />
                    )}
                  </div>
          ...
          {selectedItem && (
            <div className="flex items-center gap-2.5 bg-orange-50 dark:bg-orange-950/50 border border-orange-200 dark:border-orange-800 rounded-xl px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-orange-700 dark:text-orange-400 truncate">
                  ...
                  {chipLabel(selectedItem) !== "Sans variante" && (
                    <span className="font-normal ml-1 text-orange-600 dark:text-orange-500">
              ...
              <p className="text-sm font-bold text-orange-700 dark:text-orange-400 shrink-0">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-100 dark:border-stone-800 p-5">
              ...
              {REASONS.map((r) => (
                <label
                  ...
                  className={`flex items-start gap-2 p-3 rounded-xl border-2 cursor-pointer text-sm transition-all ${
                    reason === r.label
                      ? "border-orange-700 bg-orange-50 dark:bg-orange-950/60 text-orange-700 dark:text-orange-400 font-semibold"
                      : "border-stone-100 dark:border-stone-800 text-stone-600 dark:text-stone-400 hover:border-stone-200 dark:hover:border-stone-700"
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  {reason === r.label && (
                    <Check className="w-3.5 h-3.5 ml-auto shrink-0 text-orange-700 mt-0.5" />
                  )}
                </label>
              ))}
            </div>
            {!reason && (
              <p className="text-xs text-orange-500 dark:text-orange-400 flex items-center gap-1 mt-2">
                <AlertCircle className="w-3.5 h-3.5" /> Sélectionnez un motif
                pour continuer
              </p>
            )}
          </div>
                  ...
                  <label
                    ...
                    className={`flex flex-col items-center text-center p-4 rounded-xl border-2 cursor-pointer transition-all ${
                      resolution === res.value
                        ? "border-orange-700 bg-orange-50 dark:bg-orange-950/60 shadow-sm shadow-orange-100 dark:shadow-orange-900/20"
                        : "border-stone-100 dark:border-stone-800 hover:border-stone-200 dark:hover:border-stone-700"
                    }`}
                  >
                    <input
                      ...
                      onChange={() =>
                        setResolution(res.value as typeof resolution)
                      }
                      className="sr-only"
                    />
                    <div
                    ...
                    >
                      <Icon
                        className={`w-5 h-5 ${resolution === res.value ? "text-orange-700 dark:text-orange-500" : "text-stone-500"}`}
                      />
                    </div>
                    <span
                      className={`text-xs font-bold ${resolution === res.value ? "text-orange-700 dark:text-orange-400" : "text-stone-700 dark:text-stone-200"}`}
                    >
                      {res.label}
                    </span>
                    ...
                    {resolution === res.value && (
                      <Check className="w-3.5 h-3.5 text-orange-700 mt-1.5" />
                    )}
                  </label>
                );
              })}
          ...
          <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-100 dark:border-stone-800 p-5">
            ...
            <textarea
              ...
              className="w-full text-sm border border-stone-200 dark:border-stone-700 rounded-xl px-3 py-2.5 bg-white dark:bg-stone-800 text-stone-800 dark:text-stone-100 placeholder-stone-400 resize-none focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-700 dark:focus:border-orange-500 transition"
            />
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-100 dark:border-stone-800 p-5">
            {[
              ...
              <div
                key={k}
                className="flex justify-between items-center py-2.5 border-b border-stone-50 dark:border-stone-800 last:border-0"
              >
                <span className="text-xs text-stone-400 shrink-0">{k}</span>
                <span className="text-sm font-semibold text-stone-800 dark:text-stone-100 text-right ml-4">
                  {v}
                </span>
              </div>
            ))}
      ...
      <div className="flex gap-3 mt-6">
        {step > 0 && (
          <button
            type="button"
            onClick={prev}
            className="flex-1 border-2 border-stone-200 dark:border-stone-700 text-stone-600 dark:text-stone-300 text-sm font-semibold py-3.5 rounded-xl hover:bg-stone-50 dark:hover:bg-stone-800 transition"
          >
            ← Précédent
          </button>
        )}
        ...
        {step === 3 && (
          <button
            ...
            disabled={submitting}
            className="flex-1 bg-orange-700 hover:bg-orange-800 disabled:opacity-50 text-white text-sm font-semibold py-3.5 rounded-xl transition flex items-center justify-center gap-2"
          >
            {submitting ? (
              <>
