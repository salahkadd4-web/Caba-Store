...
export default function FavorisPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12 pt-4">
      <h1 className="text-3xl font-semibold tracking-tight text-stone-900 dark:text-stone-100 mb-2">Mes Favoris</h1>
      ...
      {favoris.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-lg text-stone-500 dark:text-stone-400 mb-4">Vous n&apos;avez pas encore de favoris.</p>
          <Link href="/produits" className="inline-block bg-orange-700 hover:bg-orange-800 text-white text-sm font-semibold px-6 py-3 rounded-xl transition-colors">
          ...
          {favoris.map((favori) => (
            <div key={favori.id}
              ...
              <Link href={`/produits/${favori.product.id}`}>
                ...
                <div className="p-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-orange-700 dark:text-orange-500 mb-1">{favori.product.category.nom}</p>
              ...
              <div className="px-4 pb-4">
                <button onClick={() => handleRetirer(favori.product.id)}
                  ...
                  <XCircle className="w-4 h-4 inline mr-1" />{' '}Retirer des favoris
                </button>
              </div>
            </div>
          ))}
